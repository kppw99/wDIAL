...
ITERATION = 10
ALGOS = ['mlp', 'xgboost']  # 'svm' or 'mlp' or 'rf' (random forest) or 'bagging_svm' or 'xgboost'
BATCH_SIZE = [2, 3, 4, 5, 6]  # 한 번에 몇 datapoint씩 파싱할 것인지 선택
...
print('')
print('[#] Total Trip Count of Train: ', len(os.listdir(train_path)))
print('[#] Total Trip Count of Test: ', len(os.listdir(test_path)))

print('[#] Total Test Dataset:', os.listdir(test_path))
...
def do_active_learning(algo, iter_cnt, batch_size):
    for mode in ['uncert', 'cert', 'random']:  # 'random' and 'distance'
        acc_list = list()
        prec_list = list()
        rec_list = list()
        f1_list = list()
        ...
        uncertainty = True if mode == 'uncert' else False
        if mode != 'random':
        ...
        print('[*] Length of pool_merged:', len(pool_merged))
        print('[*] Length of pool_keys:', len(pool_keys))
        print('############################################\n')

        # pool에 남아있는 개수 / 한 번에 몇개씩 pool에서 뽑을지?
        loop_cnt = len(pool_keys) // batch_size + 1 if len(pool_keys) % batch_size == 0 else len(pool_keys) // batch_size + 2
        for iteration_idx in range(loop_cnt):
            print('-------------------------------------------------')
            ...
            if iteration_idx == 0:
                ...
                if mode == 'random':
                    ...
                    selected_keys = pool_keys[:batch_size]
                else:
                    ...
                    for pool_key in pool_keys:
                        ...
                        if algo == 'svm':
                            ...
                            print('Please use proper algo!')
                            exit(1)
                        pool_dist_dict[pool_key] = pool_distance

                        # print(pool_key, ' : ', pool_distance)

                    # batch_size 개수만큼 데이터를 추출해둠 (거리가 먼 것 부터 선택)
                    if algo == 'svm':
                        # uncertainty dataset first (False) / certainty dataset first (True)
                        selected_pool = sorted(pool_dist_dict.items(), key=lambda x: x[1], reverse=True)[:batch_size]
                    elif algo == 'bagging_svm':
                        # uncertainty dataset first (True) / certainty dataset first (False)
                        selected_pool = sorted(pool_dist_dict.items(), key=lambda x: x[1], reverse=uncertainty)[:batch_size]
                    elif algo == 'mlp':
                        # uncertainty dataset first (True) / certainty dataset first (False)
                        selected_pool = sorted(pool_dist_dict.items(), key=lambda x: x[1], reverse=uncertainty)[:batch_size]
                    elif algo == 'rf':
            ...
            if algo == 'svm':
                ...
                model, acc, prec, rec, f1 = xgboost(train_x, train_y, test_x, test_y)
            else:
                print('Please use proper algo!')
                exit(1)
            acc_list.append(acc)
            ...
            if iteration_idx != 0:
                batch_data_list.append(selected_keys)
        ...
        filename = algo + '_' + mode + '_batch_size_' + str(batch_size) + '_' + str(iter_cnt) + filetime + '.pkl'
...
for algo in ALGOS:
    for batch in BATCH_SIZE:
        for iter_cnt in range(ITERATION):
            do_active_learning(algo, iter_cnt, batch)